...
const (
	border = 0xF_FF_FF_FF_FF_FF_FF - 1
	large  = 0xF_FF_FF_FF_FF_FF_FF
)
...
const testNewErrorError = "Test New Error %d: got \"%s\" error (should be " +
	"\"%s\")"

func TestNewError(t *testing.T) {
	for i, tt := range newErrorTests {
		_, result2 := struct64.New(tt.tree)
